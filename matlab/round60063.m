function [Y,pns,edg,idx] = round60063(X,ser)
...
% adjacent E-Series values, which match the optimal component tolerances.
%
% Syntax:
%  Y = round60063(X,ser)
...
% selected by input <ser>. <pns> is the contiguous list of E-Series values
...
%  ans = 510
%
% round60063([514, 1.9; 7.6, 37], 'E12')
%  ans = [560, 1.8; 8.2, 39]
%
% round60063([514, 1.9; 7.6, 37], 'E6')
%  ans = [470, 2.2; 6.8, 33]
%
% [Y,pns,edg,idx] = round60063([514, 1.9; 7.6, 37], 'E3')
%  Y   = [470, 2.2; 10, 47]
...
%  ser = String, to select the E-series. One of E3/E6/E12/E24/E48/E96/E192.
% Outputs:
%  Y   = Numeric, the same size as <X>, with values rounded to <ser> E-Series.
%  pns = Numeric Vector, the consecutive E-Series values encompassing <Y> values.
%  edg = Numeric Vector, the bin edges derived from adjacent <pns> values.
%  idx = Numeric, the same size as <X>, the index such that Y = pns(idx).
%
% Inputs  = (X,ser)
% Outputs = [Y,pns,edg,idx]

assert(isnumeric(X),'First input <X> must be a numeric matrix/vector/scalar.')
assert(isreal(X),'First input <X> must contain real values only.')
assert(ischar(ser)&&isrow(ser),'Second input <ser> must be a token string.')
%
...
switch ser
    ...
    otherwise
        error('Second input <ser> token is not supported: "%s"',ser)
end
%
